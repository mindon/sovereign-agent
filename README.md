...
| `src/memory.zig` | 分层信任记忆：置信度分层、`fetchSeeds`、置信度演进、`<confidence_stats>`、Contextual Exception；**持续学习/本能烧录**（晋升/阻尼/解除）、`fetchSeedsBlended`（融合环境足迹）、`instinctReflex`（见 §9.2/§9.3）。 |
| `src/arbiter.zig` | 仲裁与校验闭环：敏感操作同步强校验、非敏感操作异步预校验；**包容式行为栈 `BehaviorStack`**（L0 安全包络 / L1 资源约束 / L2 记忆启发式，低层反射优先抢占，见 §9.1）、可注入本能反射 `Layer`、可插拔 `Probe`。 |
...
- **离线黑盒测试**（本节重点）：`sabotage` / `cognitive_shift` / `ledger_replay` + 四范式 `subsumption` / `instinct` / `stigmergy` / `routing` —— 不依赖网络/LLM、确定性、带断言与退出码，**纳入 CI**（`zig build examples`）。
...
下面七个**端到端、离线、确定性**的黑盒测试，从外部把内核当作黑盒来验证核心论断。均**不依赖网络/LLM**（在 CI 中可稳定复跑），各自输出独立的 PASS/FAIL 报告与退出码（`harness.zig` 提供轻量断言框架 `Checker`）。前三者验证基础三支柱，后四者验证 §9 的四范式增强。
...
> 七个黑盒测试共 75 项断言**全部通过**。前三者对应"非完美记忆 + 零信任校验"、"置信度演进驱动的元认知"、"状态不可变 + 确定性重放审计"三条基础哲学；后四者对应 §9 的四范式增强，是对 §1 原则与进阶认知架构的可执行证明。
...
# 运行全部七个端到端黑盒测试（离线，不依赖网络；含 §9 四范式）
zig build examples
